...
return [...text.matchAll(/@([0-9]{5,16}|0)/g)].map(v => v[1] + '@s.whatsapp.net')
			...
			function addMetadata(packname, author) {	
				if (!packname) packname = 'Bot'; if (!author) author = 'By Riski Ganz';	
...
//━━━━━━━━━━━━━━━[ BUAT BIO BOT ]━━━━━━━━━━━━━━━\\
runi = process.uptime() 
           dit.setStatus(`Ganz Botv2 Aktif Selama ${kyun(runi)} `).catch((_)=>_);
...
anu =`
Hai bang maaf menu masih jelek 🗿

☯ Tampilan Menu Ganz Bot ☯
𝑷𝒓𝒆𝒇𝒊𝒙 : MULTI & PREFIX
𝑽𝒆𝒓𝒔𝒊𝒐𝒏 : Termux Online
...
┃❆${prefix}bear {teks}
└──────────────`
sendButImage(from, anu,`Hai beban \n${Tanggal}`, thumb, [
...
case 'script':
if (isBanned) return reply(mess.banned)
reply2(`Menggunakan Script dari channel Riski Ganz\nhttps://youtu.be/QPSiq5Ygg-I`)
break
//══════════[ ASUPAN MENU ]════════════════════════════//
             case 'asupangeayubi':
             if (isBanned) return reply(mess.banned)
...
//══════════[ANIME MENU]════════════════════════════//
            ...
            if (isBanned) return reply(mess.banned)
              ...
              contentText:`Creator Riski Gz`,buttons,headerType:4}
...
//══════════[ ANIMASI HEWAN ]════════════════════════════//
                   ...
                   dit.sendMessage(from, anu1, image, {caption: `nih kak hasilnya`, quoted: froxx})
                   break
                   case 'dog':  
                   if (isBanned) return reply(mess.banned)
...
//══════════[ Sticker Menu ]════════════════════════════//
          ...
          if (isBanned) return reply(mess.banned)
           ...
           dit.sendMessage(from, buffer, sticker, { quoted: mek })
			...
			fs.unlinkSync(media)
			if (err) return reply('Yah gagal banh, coba ulangi ☕')
...
dit.sendMessage(from, mp4, video)
break
case 'mp3':
if (isBanned) return reply(mess.banned)
reply(mess.wait)
...
//══════════[ WELCOME MENU ]════════════════════════════//
case 'intro':
var intro = ` *ᴊᴀɴɢᴀɴ ʟᴜᴘᴀ ɪɴᴛʀᴏ ʙᴀɴʜ 🗿☕*
...
dit.sendMessage(from, buff, image, { quoted: froxx, caption: '©Creator By Riski Ganz' })
break
//══════════[ BERMAIN MENU ]════════════════════════════//
case 'nickepep':
if (isBanned) return reply(mess.banned)
...
if (isBanned) return reply(mess.banned)
      ...
      if (isBanned) return reply(mess.banned)
             ...
             reply(mess.wait)
             exec(`node start.js`)
             reply('_Restarting Bot Success yoi _')
             break
      case 'leaveall':
      if (isBanned) return reply(mess.banned)
             ...
             sendMess(id, 'Byee bang ganteng', null)
             await sleep(3000)
             dit.groupLeave(id)
}
             break
...
if (isBanned) return reply(mess.banned)
			if (!isGroup) return reply(mess.only.group)
					...
					if (Number(args[0]) === 1) {
						...
						reply('Sukses mengaktifkan fitur welcome di group ini done banh ✔️')
					} else if (Number(args[0]) === 0) {
						welkom.splice(from, 1)
						fs.writeFileSync('./database/welkom.json', JSON.stringify(welkom))
						reply('Sukses menonaktifkan fitur welcome di group ini done banh ✔️')
					} else {
        ...
        break;
                ...
                if (isBanned) return reply(mess.banned)
					...
					if (Number(args[0]) === 1) {
						...
						reply('Sukses mengaktifkan anti link group di group ini done banh ✔️')
						...
						fs.writeFileSync('./database/antilink.json', JSON.stringify(antilink))
						reply('Sukes menonaktifkan anti link group di group ini done banh✔️')
					} else {
...
dit.sendMessage(from, 'Waalaikumsalam, stay halall brother 🗿☕', text, {quoted: mek})
                  }
if (budy.includes(`Bot`)) {
dit.sendMessage(from, 'Ada apa?, btw napa panggil gw 🗿', text, {quoted: mek})
                  }
if (budy.includes(`Tes`)) {
dit.sendMessage(from, 'napa', text, {quoted: mek})
                  }
}
